# Agent 1B — `llama3.2:1b`

The smallest agent in the set. Same code, same tools, same harness as the other
four folders; only `config.json` differs.

```json
{ "name": "Agent 1B", "model": "llama3.2:1b", "num_ctx": 8192 }
```
...
the parts a 1B model cannot be trusted to do on its own.
...
**Take this seriously at 1B.** A model that struggles to put the right number in
the right spreadsheet cell will eventually issue a confidently wrong
`delete_path`. Point `--root` at a sandbox, not at anything you care about.
...
.\run.ps1 --tiers --deep qwen2.5:14b "..."
```

At 1B, `--small` is pointless — there is nothing cheaper to route down to. The
tiers flag is useful here mainly for the per-role accounting, or to hand
planning and verification to something larger while the 1B keeps driving:
...
See [`training_scripts/`](../../training_scripts/) — that trainer targets
**exactly this model** (Llama 3.2 1B), and the `1B` vs `1B + adapter` delta on
the 12 benchmark tasks is the experiment it exists to run.
...
## 8. What to expect at 1B

Fast — genuinely interactive on CPU, seconds per step, the only size in the set
you can iterate on comfortably.

It is also the size the harness was designed against. Nearly every mechanism
above exists because of a failure mode this model exhibits: JSON that is almost
valid, parameter names that are almost right (`start` for `start_time`,
`when` for `date`), the same call issued four times in a row, `done` announced
before anything has been written. Turning the scaffolding off — which is what
the `raw` condition in [`bench/`](../../bench/) does — is the measurement, and
the gap between the two conditions is widest here.

Keep tasks to one or two steps. Multi-part instructions ("read the email, then
add the event, then reply") are where 1B drops a clause; the verifier catches
